...
1. **Reranker in MVP?** Default: no (RRF only). If the demo shows weak ranking on runbooks, add a local cross-encoder before Phase 7 eval. Confirm.
...
- Measured retrieval quality below target in Phase 7 ⇒ add reranker + query rewriting before shipping the eval dashboard.
