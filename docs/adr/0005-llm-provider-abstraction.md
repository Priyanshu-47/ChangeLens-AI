...
Model names are configuration (`GEMINI_TEXT_MODEL`, `GEMINI_EMBEDDING_MODEL` in `.env.example`), validated at startup/readiness against the API so a deprecated model fails fast. The default is a current GA model; the eval gate (Phase 7) detects quality regressions if the model is swapped.
