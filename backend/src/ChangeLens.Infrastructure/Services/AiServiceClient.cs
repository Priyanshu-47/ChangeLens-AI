    ...
    ILogger<AiServiceClient> logger) : IAiServiceClient
{
    ...
    public async Task<RetrievalSearchResponseDto> RetrievalSearchAsync(
        RetrievalSearchRequestDto request, CancellationToken ct)
    {
        var correlationId = ResolveCorrelationId();

        var httpRequest = new HttpRequestMessage(HttpMethod.Post, "internal/v1/retrieval/search")
        {
            Content = JsonContent.Create(request, options: Json)
        };
        httpRequest.Headers.Add("X-Internal-Key", options.Value.ApiKey);
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                return JsonSerializer.Deserialize<RetrievalSearchResponseDto>(body, Json)
                    ?? throw new AiUnavailableException("AI service returned an empty response body.");
            }
        ...
        {
            logger.LogWarning("Retrieval search timed out after {Timeout}s (correlation {CorrelationId})",
                options.Value.TimeoutSeconds, correlationId);
            throw new AiTimeoutException(
                $"AI service did not respond within {options.Value.TimeoutSeconds}s.");
        }
